...
So because we create src-gen which is our actual classpath...
Plugins need some way of actually using the upstream as a
proper dependency.

So we need to enable the dependency library to be able to
force the generation of src-gen directly.
