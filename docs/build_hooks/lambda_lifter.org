...
For now this is more so an experimental attempt to perhaps simplify
the Hiccup writing so that contextually the library need not be
aware of what an mxc link is versus an https.

For now it is mostly unused.

#+begin_src clojurescript :tangle ../hooks/lambda_lifter.clj
(defn rewrite-custom-rules [code]
  (let [zloc (z/of-string code {:track-position? true})
        root-zloc (or (z/up zloc) zloc)
        mod-zloc (z/postwalk
                  root-zloc
                    ...
                    (and (= :vector (z/tag loc))
                         (let [fst (z/down loc)]
                           (and fst
                                ...
                                (= :img (z/sexpr fst))))))
                  (fn [loc]
                    (-> loc z/down (z/replace 'mxc-media) z/up)))]
    (z/root-string mod-zloc)))
#+end_src
  ...
  (let [raw-code (slurp file)]
    (if (skip-file? raw-code)
      ...
      (let
       ...
       [code (rewrite-custom-rules raw-code)
        reqs (cond-> []
               (clojure.string/includes? code "mxc-media") (conj '[utils.images :refer [mxc-media]]))

        {:keys [new-source registry]} (rewrite-lambdas code)
        ...
        instrumented (instrument-host-file ast-form reqs)]
