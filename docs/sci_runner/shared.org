...
This is for namespaces that would uniformly
be shared across the SCI runner instances.
Each SCI runner namespace will live on
the respective thread identity and as such
we want to make reduce the duplicity if we
can.

**** Imports


#+begin_src clojurescript :tangle ../../src/sci_runner/shared.cljs
(ns sci-runner.shared
