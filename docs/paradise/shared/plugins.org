...
Currently the implementation is functional, but some improvements are needed.
The improvements, however, would require namespace changes and some re-organization
so for now we'll continue with this simpler mechanism.

At a later time the goal would be to instead prefix with paradise to the namespace
so we can more easily expose to SCI which namespaces we should expose.
Then building on that we need to expand the threading identities to cover the
namespaces more clearly.
Many files in the project belong to the UI scope and a large chunk of the others
belong to the engine scope (worker).

The remaining two thread identities are media and virtualizer.
The virtualizer ideally would be lifted away from the repo when the thread
solution is more finalized, and the media pool is rather specific (but should
be exposable).

Then we can create an easier mechanism for loading plugin functions into threads
because they can target FQN and the FQN can be associated with certain
thread identities. So for say the engine pool to be added to we'd then be able to
do something like
(ns paradise.engine.some-plugin)

and it'd be associated with the engine thread innately.
...
#+begin_src clojurescript :tangle ../../../src/paradise/shared/plugins.cljs
...
To expand on that, we've found that we can't exactly do a wrapper
because once the Google Closure library runs on the code...
the results are mangled to a point where there isn't anything
useful in them. The SmallTalk pattern might be usable, but I
don't believe it is inherently needed... I instead opted
for a defoverride pattern and we'll define UI components
with a defui macro that builds a registry. Then it is
much more trivial to actually replace the component directly.
...
#+begin_src clojurescript :tangle ../../../src/paradise/shared/plugin_storage.cljs
