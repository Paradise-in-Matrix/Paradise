...
#+begin_src clojurescript :tangle ../../../../src/paradise/shared/utils/helpers.cljs
...
***** TODO transform-img is still using lazy string coercion
To elaborate on the above, this just means the dehydrator isn't accounting for
maps and so when it is sent over the thread boundary we have an object in the
'clean' hiccup form. It works correctly as the worker-pojo is in the right form,
but as a hacky fix we'll just leave the raw string coercion in since it doesn't
throw an error.
...
Important to note
eve more or less obliterates any buffers or pointers to buffers
so we must coerce it into a pure CLJS object instead
at some point it may be worth looking at fixing this
in the library itself

This wasn't entirely obvious through logs, so a multitude
of approaches for passing the SAB were written...
For now, we'll make sure the coercions are obvious


The following note was from before we moved this code:
...
***** Attachment/File Registry
Buffers don't play well with being in the app-db, so we need to create a
spot for them to live and be utilized neatly outside of it.
