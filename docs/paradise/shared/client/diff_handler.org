...
#+begin_src clojurescript :tangle ../../../../src/paradise/shared/client/diff_handler.cljs
...
The Rust SDK more or less only uses the following options for diffs, so it
is sensible to handle all the potential diff options in one location in
a generic manner.
