...
Handling Timelines and stuff from the Rust SDK
(If the files have simple bits like here... they've likely
been shuffled around a bunch and this portion existed after
most of it was written).

**** Imports

#+begin_src clojurescript :tangle ../../../src/paradise/engine/timeline.cljs
...
We actually don't want to render on the main thread, so we send this
instead to the virtualizer pool.
It may be reasonable eventually to reduce the executed logic on the
main thread even more and just make a 'rendering' pool, but for now
we'll be sticking with just doing the virtualizer this way.
...
The Rust SDK has a method for this, but the FFI doesn't have it bound last I checked.
For now we're going to just stub the mechanism and fix it later.

***** TODO Add singular event fetching outside of timeline scope
