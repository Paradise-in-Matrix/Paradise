...
Now we need to refresh certain components of the room. We don't inherently want all of this info
and the rust-sdk mentions that subscribing to these for every room would be expensive. So
this would be most sensible to only do when the user explicitly clicks on a room.

Small note, but as I wasn't feeling great I wrote this in isolation.
We do have a map with unfiltered cache, but I didn't want to check if the sync would wipe
metadata we append in. For now we'll create a separate structure to store the data.
Later we should more cleanly unify the two (like having a submap in the room-id named respectively
for each one's contents. So room-sync-data and room-info-data or something like that.

We did test this out and in the current version of the rust-sdk ALWAYS reports
an empty array back to us for pinnedEvents... which is useless. This code may be
more useful later when we can update our rust-sdk.
