...
(re-frame/reg-sub
 :room/is-call?
 (fn [db [_ room-id]]
   (let [preview   (get-in db [:room-previews room-id])
         room-type (:room-type preview)]
     (if (map? room-type)
       (and (= "Custom" (:tag room-type))
            (= "org.matrix.msc3417.call" (get-in room-type [:inner :value])))
       (and (= "Custom" (some-> room-type .-tag))
            (= "org.matrix.msc3417.call" (some-> room-type .-inner .-value)))))))
#+end_src
...
(defn render-room-item [tr client rooms active-space active-room closed-drawers active-filter]
  (fn [_ raw-room]
    (let [{:keys [id roomId name is-space? isSpace is-direct? isDirect depth
