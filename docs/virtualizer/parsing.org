...
#+begin_src clojurescript :tangle ../../src/virtualizer/parsing.cljs
(ns virtualizer.parsing
  (:require ["@xmldom/xmldom" :as xmldom]
            ...
            [utils.helpers :refer [process-raw-event sanitize-custom-html linkify-text]]))
#+end_src
    ...
    (if (or (keyword? (first node))
      ...
      (let [tag-kw (first node)
            tag-name (cond
                       ...
                       (fn? tag-kw) (or (.-displayName tag-kw) (.-name tag-kw) "unknown-fn")
                       :else "div")
