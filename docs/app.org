...
(ns app
  (:require
   ...
   [client.config :refer [load-config check-remote-version load-i18n eve-enabled?]]
   [client.state :refer [!config]]
   [client.session-store :as store]
   [taoensso.tempura :as tempura :refer [tr]]
   [utils.global-ui :refer [modal-root popover-root context-menu-root satellite-overlay make-swipe-handlers]]
