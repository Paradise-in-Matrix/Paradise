...
So this can be broken into two parts. As the worker threads would be unable to communicate
with the main thread they actually need to instantiate their own SCI context.

With that in mind we need to define the shared entries between the contexts
...
#+begin_src clojurescript :tangle ../src/sci_shared.cljs
...
#+begin_src clojurescript :tangle ../src/worker_sci_runner.cljs
