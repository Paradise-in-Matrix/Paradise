...
(defn test-timeline [room-id]
  ...
  (r/with-let [!scroll-ref      (atom nil)
               ...
               !initialized?    (r/atom false)
               !scroll-timer    (atom nil)
#_               item-resize-obs  #_(js/ResizeObserver.
                                 (fn [entries]
                                   (doseq [entry entries]
                                     (let [el (.-target entry)
                                           id (.getAttribute el "data-event-id")
                                           ...
                                           h (.-height rect)]
                                       (when (and id (pos? h))
                                         (let [current (get @!measured id)]
                                           (when (or (nil? current) (> (js/Math.abs (- current h)) 1))
                                             (swap! !measured assoc id h))))))))
                  ...
                  (fn [entries]
                    (doseq [entry entries]
                      (let [el (.-target entry)
                        ...
                        (when (and id (pos? dom-h))
                          (let [current-measured (get @!measured id)
                                estimated (estimate-height {:id id} @!container-width !prepared-cache !measured @!theme-metrics)
                                diff (- dom-h estimated)]
                            (when (> (js/Math.abs diff) 2)
                              (log/warn "JITTER DETECTED on" id
                                        "| Estimated:" estimated
                                        "| Actual DOM:" dom-h
                                        "| Diff:" diff))
                            (when (or (nil? current-measured) (> (js/Math.abs (- current-measured dom-h)) 2))
                              (swap! !measured assoc id (js/Math.round dom-h)))))))))
               container-obs    (js/ResizeObserver.
                                 (fn [entries]
                                   (let [rect (.-contentRect (aget entries 0))]
