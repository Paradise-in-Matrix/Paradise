...
#+begin_src clojurescript :tangle ../../../src/container/timeline/hydrator.cljs
(ns container.timeline.hydrator
  (:require [goog.object]
            ["react" :as react]
            [reagent.core :as r]
            [client.state :refer [!ast-handoff]]
  ...
  (let [k (if (string? raw-ptr) raw-ptr (str raw-ptr))]
    (if-some [cached (goog.object/get !gen-fn-cache k)]
      ...
      (let [clean-n (if (.startsWith k ":fn/") (.substring k 4)
            ...
            gen-fn  (get @client.registry/!anon-fns kw-ptr)]
...
(def sanitize-js-dispatch
  (re-frame/->interceptor
   ...
   :before (fn [context]
             (update-in context [:coeffects :event] container.timeline.hydrator/fast-js->clj-dmp))))
...
(defn resolve-react-type [type-str]
  (if-some [cached (goog.object/get !tag-cache type-str)]
    (if (keyword? cached)
      (or (:fn (get @client.registry/!active-overrides cached)) (get @client.registry/!components cached) "div")
      cached)
    (cond
      ...
      (let [kw (fast-keyword (subs type-str 5))]
        ...
        (or (:fn (get @client.registry/!active-overrides kw)) (get @client.registry/!components kw) "div"))
