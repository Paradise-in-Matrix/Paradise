...
(defn boot-timeline! [{:keys [source room room-id focus-obj track-receipts paginate-amount]}]
  ...
  (-> (p/let [is-live? (= source :live)
              ...
              _        (when (:forward paginate-amount)
                         (.paginateForwards tl (:forward paginate-amount)))
...
(worker/register :cleanup-timeline
  (fn [{:keys [room-id]}]
    ...
    (swap! !timeline-mutexes dissoc [room-id :live] [room-id :focused])
    {:status :success}))
...
(defn boot-live-pin! [room room-id event-id]
  (go
    (try
      (let [focus-obj (focus {:type :event :id event-id :count 1})
            config    (.create TimelineConfiguration
                               #js {:focus             focus-obj
                                    :dateDividerMode   (.. sdk -DateDividerMode -Daily)
                                    :filter            (new (.. sdk -TimelineFilter -All))
                                    :trackReadReceipts 0})
            timeline  (<p! (.timelineWithConfiguration room config))
            handle    (.addListener timeline
                        #js {:onUpdate
                             (fn [_diffs]
                               (-> (.getEventTimelineItemByEventId timeline event-id)
                                   (p/then (fn [item]
                                             (when item
                                               (let [parsed (wrap-item (make-timeline-item-shim item event-id))]
                                                 (worker/stream! {:type    "pin-update"
                                                                  :room-id room-id
                                                                  :event   parsed})))))
                                   (p/catch #(log/error "Live pin resolve failed:" %))))})]
        (let [source-key (keyword (str "pin-" event-id))]
          (swap! !active-timelines assoc-in [room-id source-key] {:timeline timeline :handle handle})))
      (catch :default e
        (log/error "Failed to boot live pin:" event-id e)))))
      ...
      (if-let [room (.getRoom @state/!client room-id)]
        (try
          (let [client   @state/!client
                ...
                clean-hs (str/replace (.-homeserverUrl session) #"/+$" "")
                url      (str clean-hs "/_matrix/client/v3/rooms/" room-id "/state/m.room.pinned_events")
                resp     (<p! (net/fetch url #js {:headers #js {"Authorization" (str "Bearer " token)}}))]
            (if-not (.-ok resp)
              (worker/stream! {:type "pins-sync" :room-id room-id :pinned-ids []})
              (let [state-json (<p! (.json resp))
                    pinned-ids (get (js->clj state-json :keywordize-keys true) :pinned [])]
                (worker/stream! {:type "pins-sync" :room-id room-id :pinned-ids pinned-ids})
                (doseq [id pinned-ids]
                  (boot-live-pin! room room-id id)))))
          {:status :success}
          (catch :default e
            {:status :error :msg (str e)}))
        {:status :error :msg "Room not found"}))))
