  ...
  (let [tr               @(re-frame/subscribe [:i18n/tr])
    ...
    (fn []
      (let [installed-urls @(re-frame/subscribe [:plugins/installed-urls])
        ...
        [:div.plugin-installer-panel {:style {:display "flex" :flex-direction "column" :gap "24px"}}

         [:div.plugins-container-card.split-layout
          [:div.install-column
           [:h3.settings-subheading {:style {:margin-top 0}} (tr [:settings.plugins/themes-title])]
           ...
           (when custom-theme?
             [:div.active-theme-card
              [:div.active-theme-info
               [:span.active-theme-label (tr [:settings.plugins/active-custom-theme])]
         ...
         [:div.plugins-container-card.split-layout
          [:div.install-column
           [:h3.settings-subheading {:style {:margin-top 0}} (tr [:settings.plugins/plugins-title])]
           [:div.plugin-input-row {:style {:flex-direction "column"}}
            [:input.form-input.plugin-input
             {:type "text"
          ...
          [:div.list-column
           (when (seq active-plugins)
            [:<>
             [:h4.active-plugins-header {:style {:margin-top 0 :padding-top 0 :border-top "none"}}
              (tr [:settings.plugins/active-plugins])]
