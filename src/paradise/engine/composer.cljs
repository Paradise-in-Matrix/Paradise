(ns paradise.engine.composer
  (:require [cljs.core.async :refer [go]]
            ...
            [paradise.engine.timeline :refer [!active-timelines]]
                       ...
                       (if-let [client @state/!client]
                         (let [session (.session client)
                           ...
                           (if (= source-type "account")
                             (let [rooms-json  (<p! (.accountData client "im.ponies.emote_rooms"))
                                   rooms-data  (when rooms-json (js->clj (js/JSON.parse rooms-json) :keywordize-keys true))
                                   targets     (if rooms-data
                                                 (for [[rid packs] (:rooms rooms-data)
                                                       [pack-name _] packs]
                                                   [(name rid) (name pack-name)])
                                                 [])
                                   remote-results (<p! (p/all (map (fn [[r p]]
                                                                     (p/then (fetch-state r "im.ponies.room_emotes" p)
                                                                             #(when % {:pack-id p :data %})))
                                                                   targets)))
                                   personal-json (<p! (.accountData client "im.ponies.user_emotes"))
                                   personal-pack (when personal-json
                                                   (let [data (js->clj (js/JSON.parse personal-json) :keywordize-keys true)]
                                                     {:pack-id "user-emotes" :data data}))
                                   all-results (cond-> (vec (remove nil? remote-results))
                                                 personal-pack (conj personal-pack))]
                               {:status :success :packs all-results})
